...
class CloudUser {
  ...
  async fetchOrgDataAndUpdate() {
    ...
    if (appData && appData.Item && Object.keys(appData.Item.apps).length > 0) {
      ...
      const verificationData = await dc.walletAnalyticsDataTableGet(
        currentAppId
      );
      try {
        const verified =
          Object.keys(verificationData.Item.analytics).length > 0;
        setGlobal({ verified });
      } catch (e) {
      ...
      if (data.currentSegments && data.currentSegments.length > 0) {
      ...
      if (liveChat) {
        ...
        const mainThreadHash = mainThread.address
          .split("orbitdb/")[1]
          .split("/3box")[0];
        mainThreadPosts = await getPosts(mainThread);
        fetchAllPosts(space, mainThreadPosts);
  ...
  async fetchUsersCount(appData) {
    ...
    const matchingSegment = currentSegments
      ? currentSegments.filter((a) => a.id === defaultSegmentId)
      : [];
...
function getChatSupportAddress() {
  ...
  return new Promise((resolve, reject) => {
    const chatWallet = localStorage.getItem(CHAT_WALLET_KEY)
      ? JSON.parse(localStorage.getItem(CHAT_WALLET_KEY))
      : undefined;
    if (chatWallet) {
    ...
    try {
      ...
      if (firstModAddress) {
        thread = await space.joinThread(threadId, {
          firstModerator: firstModAddress,
        });
      } else {
        thread = await space.joinThread(threadId);
      }
