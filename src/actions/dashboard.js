export function loadCharts() {
  ...
  window.$('#blog-overview-date-range').datepicker({});
  ...
  var bouCtx = document.getElementsByClassName('background-chart');

  for(const chart of bouCtx) {
    // Data
    var bouData = {
    ...
    if(chart) {
      window.BlogOverviewUsers = new window.Chart(chart, {
        type: 'LineWithLine',
        data: bouData,
        options: bouOptions
      });
  ...
  if(window.BlogOverviewUser) {
    var aocMeta = window.BlogOverviewUsers.getDatasetMeta(0);
